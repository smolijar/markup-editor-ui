...
Dekstopový (Windows, macOS, Linux) komerčný markdown-only (GitHub flavored) editor s live preview dokumentu v rovnakom okne hneď počas písania. link:+https://typora.io+[https://typora.io]
...
* Outline 
  ...
  ** UX fail: linky len na texte, nie na celom riadku outline.
  ** Pevná šírka 
* Podpora viacerých tabov (UX fail: ale každý dokument ako aj nápovedu vyvolanú z menu otvorí v novom okne)
* UX fail: defaultne neexistuje klávesová skratka na otvorenie nového tabu s dokumentom (štandardne cmd+t)
* Chýbajú presety klavesových skratiek 
...
* Nepodporuje režim so zobrazeným zdrojákom a vyrenderovaným dokumentom vedľa seba
  ** štandardne live rendering v tom istom okne (preview)
...
* Práca sa tabuľkami
  ...
  ** Malý toolbar pre výber zarovnania textu v bunkách tabuľky a pohodlný resize
...
(viac menej asi popísané vyššie)

* Pekný syntax highlighting kódu
...
* Export do PDF, docx, OpenOffice, LaTeX, MediaWiki, Epub

==== Shrnutí

===== Plusy
* Príjemný vzhľad, možnosť zmeny vzhľadu
...
* Spellchecker nepodtrháva nesprávne slovo
...
Jedná sa o webovú komponentu, ktorá ale nie je embedovateľná. Podporuje iba jazyk AsciiDoc. Kvôli chýbajúcej command pallete vyžaduje používanie myši ak si užívateľ nepamätá potrebnú klávesovú skratku. Podporuje zmenu vzhľadu pomocou css šablón. Dokument je možné uložiť na Dropbox, Google Drive alebo na filesystém. 
...
* Okno editoru je rozdelené na dve časti
  ...
  ** vyrenderovaný dokument (live rendering)
  ** možnosť zapnúť synchronizované scrollovanie v nastaveniach


* Klavesové skratky
  ** možnosť vybrať z 3 presetov (default, vim, emacs)
  ** UX fail: užívateľ nikde nezistí aké konkrétne klavesové skratky je možné používať 


==== Pozorování UI v kontextu markup-editor

* má Toolbar(menu) v hornej časti
  ** Otvorenie dokumentu (z Dropboxu, GG Drive, filesystem) alebo vytvorenie nového dokumentu
  ** uloženie (GG Drive, Dropbox, filesystem)
...
* Nie je možné prepínať režimy zobrazenia
  ...
  ** Žiadny distraction free mod, pri fullscreene stále vidno prvky prehliadača
* Chýba statusbar
  ** Chýba informácia o pozícií kurzora v dokumente
* Nemá outline
...
- AsciiDoc?
- Open-source
- Upozorní pred zatvorením stránky a teda stratou dokumentu (:D) 
